...
def read_atomic_structure(structure_file):
    ...
    if ('xml' in structure_file) or ('XDATCAR' in structure_file) or (os.path.isdir(structure_file) or ("OUTCAR" in structure_file)):
...
def read_energy_outcar(outcar):
    ...
    if ('OUTCAR' in outcar):
        structure.read_trajectory_vasp(outcar)
