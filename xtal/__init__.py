...
class AtTraj(object):
    ...
    def read_trajectory_vasp(self, filename):
        ...
        if '.xml' in filename:
            ...
            self.read_trajectory_vasp_aselite(filename)
        else:
    ...
    def read_trajectory_vasp_aselite(self, filename):
        traj= aselite.read_vasp_out(filename)
        for i, image in enumerate(traj):
            self.box= image.get_cell()
            self.make_dircar_matrices()
            symbols = image.get_chemical_symbols()
            natoms= len(symbols)
            snapshot = self.create_snapshot(Snapshot)
            snapshot.energy= image.get_potential_energy()
            for i in range(natoms):
                myatom= snapshot.create_atom(Atom)
                myatom.element = image.get_chemical_symbols()[i].upper()
                myatom.cart= np.array(list(map(float, image.get_positions()[i])))
                myatom.force= np.array(list(map(float, image.get_forces()[i])))
            self.cartodir()
